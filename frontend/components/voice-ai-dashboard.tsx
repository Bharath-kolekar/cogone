...
import { VoiceAIIntegration, initializeVoiceAIIntegration } from '@/services/voice-ai-integration'
...
export function VoiceAIDashboard({ className = '' }: VoiceAIDashboardProps) {
  ...
  const [aiIntegration, setAiIntegration] = useState<VoiceAIIntegration | null>(null)
  ...
  useEffect(() => {
    if (voiceDictation.isSupported && !isInitialized) {
      ...
      const integration = initializeVoiceAIIntegration(voiceDictation, commandMapper)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <VoiceDictationToggle
          onVoiceCommand={(command) => {
            console.log('Voice command in AI dashboard:', command)
          }}
          onToggle={(enabled) => {
            if (enabled) {
