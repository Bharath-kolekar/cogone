"""
Queue Service for Smart Coding AI
Preserves async task processing and queue management
"""
...
class QueueService:
    ...
    Supports priority queues and task retry logic
    """
    ...
    def _init_memory_queue(self):
        ...
        self.queues = {}  # Will be created on demand per queue name
        self.queue_items = {}
        logger.info("Memory queue initialized - queues created on demand")
    
    def _init_redis_queue(self):
        """Initialize Redis queue for distributed processing"""
        try:
            from redis import asyncio as aioredis
            redis_url = os.getenv("REDIS_URL", None) or os.getenv("UPSTASH_REDIS_URL", None) or "redis://localhost:6379"
            self.redis_client = aioredis.from_url(redis_url, decode_responses=True)
            self.queues = {}  # Metadata for Redis queues
            self.queue_items = {}
    ...
    def _init_database_queue(self):
        """Initialize database queue for persistent task storage"""
        try:
            ...
            self.queues = {}
            self.queue_items = {}
            logger.info("Database queue initialized with Supabase")
        except Exception as e:
            logger.warning(f"Database queue initialization failed: {e}, falling back to memory")
            self._init_memory_queue()
    ...
    async def enqueue(self, queue_name: str, data: Dict[str, Any], priority: str = "normal", 
        ...
        try:
            with self.lock:
                ...
                logger.debug("Item enqueued", queue_name=queue_name, item_id=item_id, priority=priority)
                return item_id
