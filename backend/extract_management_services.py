"""
Extract RBACManager, StateManager, SessionMemoryManager from smart_coding_ai_optimized.py
"""

def extract_class_by_name(lines, class_name):
    """Extract a class by finding its start and end"""
    start_idx = None
    for i, line in enumerate(lines):
        if line.startswith(f'class {class_name}:'):
            start_idx = i
            break
    
    if start_idx is None:
        return None, None, None
    
    # Find end of class
    end_idx = len(lines)
    for i in range(start_idx + 1, len(lines)):
        if lines[i].startswith('class ') and not lines[i].startswith('    '):
            end_idx = i
            break
    
    class_content = '\n'.join(lines[start_idx:end_idx])
    return start_idx, end_idx, class_content

# Read file
with open('app/services/smart_coding_ai_optimized.py', 'r', encoding='utf-8') as f:
    lines = f.read().split('\n')

# Extract classes
classes_to_extract = ['RBACManager', 'StateManager', 'SessionMemoryManager']
extracted = {}

for class_name in classes_to_extract:
    start, end, content = extract_class_by_name(lines, class_name)
    if content:
        extracted[class_name] = {
            'start': start,
            'end': end,
            'content': content,
            'lines': end - start
        }
        print(f"Found {class_name}: lines {start+1} to {end} ({end-start} lines)")

print("\nCreating files...")

# RBACManager
if 'RBACManager' in extracted:
    output = f'''"""
Smart Coding AI Management - RBAC Manager
Extracted from smart_coding_ai_optimized.py
"""

import structlog
from typing import Dict, List, Optional, Any
from datetime import datetime

logger = structlog.get_logger()


{extracted['RBACManager']['content']}


__all__ = ['RBACManager']
'''
    with open('app/services/smart_coding_ai_rbac.py', 'w', encoding='utf-8') as f:
        f.write(output)
    print(f"Created: smart_coding_ai_rbac.py ({len(output)} bytes)")

# StateManager
if 'StateManager' in extracted:
    output = f'''"""
Smart Coding AI Management - State Manager
Extracted from smart_coding_ai_optimized.py
"""

import structlog
from typing import Dict, List, Optional, Any
from datetime import datetime

logger = structlog.get_logger()


{extracted['StateManager']['content']}


__all__ = ['StateManager']
'''
    with open('app/services/smart_coding_ai_state.py', 'w', encoding='utf-8') as f:
        f.write(output)
    print(f"Created: smart_coding_ai_state.py ({len(output)} bytes)")

# SessionMemoryManager
if 'SessionMemoryManager' in extracted:
    output = f'''"""
Smart Coding AI Management - Session Memory Manager
Extracted from smart_coding_ai_optimized.py
"""

import structlog
import uuid
from typing import Dict, List, Optional, Any
from datetime import datetime, timedelta

logger = structlog.get_logger()


{extracted['SessionMemoryManager']['content']}


__all__ = ['SessionMemoryManager']
'''
    with open('app/services/smart_coding_ai_session.py', 'w', encoding='utf-8') as f:
        f.write(output)
    print(f"Created: smart_coding_ai_session.py ({len(output)} bytes)")

print("\nExtraction complete!")

